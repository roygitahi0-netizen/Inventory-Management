...
class TestInventoryAPI(unittest.TestCase):
    ...
    def test_create_item(self):
        response = self.client.post(
            "/api/inventory",
            json={"name": "Widget", "quantity": 5, "price": 3.5},
        )
    ...
    def test_external_search(self, mock_search):
        mock_search.return_value = [{"name": "Cheddar Cheese", "barcode": "999", "brand": "Dairy Co",
                                      "category": "Cheese", "image_url": None}]
    ...
    def test_import_from_barcode_adds_to_inventory(self, mock_fetch):
        ...
        response = self.client.post(
            "/api/inventory/import/barcode",
            json={"barcode": "12345", "quantity": 3, "price": 4.99},
        )
        self.assertEqual(response.status_code, 201)
        body = response.get_json()
